...
def bulk_deactivate_product(
    *, product_ids: Iterable[str], company_id: str
) -> BulkActionResult:
    result = BulkActionResult()
    normalized_ids = [str(pid) for pid in product_ids]
    products = list(
        get_products_by_ids(product_ids = normalized_ids, company_id=company_id)
    )
    ...
    for product in products:
        ...
        if not product.is_active:
            result.add_failure(item_id=pid, reason="product is not active")
            continue
